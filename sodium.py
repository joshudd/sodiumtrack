...
class SodiumDbGUI:
    ...
    def __init__(self):
        try:
            ...
            # lines
            tkinter.ttk.Separator(self.root, orient='vertical').grid(column=2, row=0, rowspan=10,
                                                                     sticky='nsw', padx=0)
            tkinter.ttk.Separator(self.root, orient='horizontal').grid(column=0, row=6, columnspan=2,
                                                                       sticky='ews', padx=0)
            tkinter.ttk.Separator(self.root, orient='horizontal').grid(column=2, row=3, columnspan=1,
                                                                       sticky='ew', padx=0)
    ...
    def add_food(self):
        try:
            ...
            cur.execute('''INSERT INTO Food (food_name, sodium_amt, carb_amt, fat_amt, calorie_amt, protein_amt)
                                VALUES (?, ?, ?, ?, ?, ?)''',
                        (self.food_name_entry.get(), float(self.sodium_entry.get()), float(self.carb_entry.get()),
                         float(self.fat_entry.get()), float(self.calorie_entry.get()), float(self.protein_entry.get())))
            # commit changes
            conn.commit()
            tkinter.messagebox.showinfo('Success!', 'The food item has been added.')
        ...
        # error when entering non-numeric values into entries
        except ValueError:
            tkinter.messagebox.showinfo('ERROR', 'Error. Please enter numbers only.')
